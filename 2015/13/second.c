...
int main() {
    ...
    PersonCount += 1; // add myself
    struct person *global_persons = calloc(PersonCount, sizeof(struct person));
    struct relation *global_relations = calloc(
        lines + ((PersonCount - 1) * 2), sizeof(struct relation)); // add my relation. which would be 8 * 2 because both goes both ways
    ...
    global_persons[PersonCount - 1].name = 0; // add my self as 0 to the last element of persons
    for (int i = 0; i < PersonCount - 1; i++) {
        global_relations[relationCount].affectedGuest = global_persons[PersonCount - 1].name;
        global_relations[relationCount].affectingGuest = global_persons[i].name;
        global_relations[relationCount].amount = 0;
        relationCount++;
        global_relations[relationCount].affectingGuest = global_persons[PersonCount - 1].name;
        global_relations[relationCount].affectedGuest = global_persons[i].name;
        global_relations[relationCount].amount = 0;
        relationCount++;
    }
